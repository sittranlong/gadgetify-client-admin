...
const handleDelete = (id) => {
  const index = addedProducts.value.findIndex(item => item.id === id);
  if (index !== -1) {
    addedProducts.value.splice(index, 1);
    message.success(`${t('delete')} ${t('product')} ${t('success')}`);
  }
};
const {n, t} = useI18n();
...
const categoryOptions = computed(() => {
  const categories = new Set(addedProducts.value.map(item => item.tenDanhMuc));
